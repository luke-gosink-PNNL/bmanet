...
\begin{abstract}
	...
	We show that this process results in a final aggregate estimate that outperforms all individual methods by reducing estimate errors by 28-91\% to a level of \todo{XYZ kJ mol$^{-1}$} accuracy.
...
%% LJG - All analysis and work was done by the listed authors 
...
\begin{table}[t]
	...
	This column indicates that the ensemble design approach presented in this work is able to reduce estimation errors by 29\% to 91\% in comparison to the individual methods.
	The final BMA ensemble is indicated by the two blue highlighted methods: imp-2 and alc-3.
	...
	\begin{tabular}{l|r|c|c|c}
		\hline
		\hline
		Reference & Method ID & Methodology & Ensemble Mean RMSE \todo{(units?)} & Performance Improvement \\
...
\begin{table}[t!]
	...
	\begin{tabular}{l|c|c|c|c|c}
		\hline
		\hline
		Stage & Ensemble mean RMSE \todo{(units?)} & Method Selected & p-value & p-value  & Performance  Benefit\\
...
\begin{figure}
	...
	\caption{This figure is one of two figures (Figures~\ref{Analysis:Figure4:Compounds1} and~\ref{Analysis:Figure5:Compounds2}) that depict the RMSE \todo{(units?)} performance of several methods based on the individual compounds taken from the SAMPL4 challenge: the first, second and third performing methods (i.e., imp-2, imp-8, and alc-3) as well as exp-3.
...
As all of these approaches construct an estimate for $\beta$, training and predicting with these approaches was performed identically to BMA, using the cross-validation detailed in Section~\ref{EP:training}.
...
\begin{figure}
	...
	The mean root mean squared error, min, max, first and third quartiles \todo{(units?)} of these ensemble approaches are shown based on the 100 iterations of the 2-fold cross-validation experiment discussed in Section~\ref{Method:StatEnsemble}.
...
\todo{Need to fix several of the references}
