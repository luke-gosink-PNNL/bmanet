...
\begin{table}[t]
	...
	\begin{tabular}{l|r|c|c|c|c}
		...
		\textit{Sandberg et al.~\cite{Sandberg:2013}} & \textit{imp-2} &hour  & \textit{multi-conformation implicit} & \textit{1.15 $\pm$ 0.023} & \textit{29\%}\\
...
The final observation is that this framework has found a single, consistent method to combine two methods to form a more optimal method for estimating hydration free energies.
These methods are based on two different approaches for modeling: one is based on a multi-conformational implicit methods and requires tens of minutes to make a prediction, where as the other is based on molecular dynamics and requires many hours to make a prediction. 
