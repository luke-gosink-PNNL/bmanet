...
Intuition may suggest that aggregated estimate performance is based on the degree that training and testing molecules come from the same chemical space.
For example, if there is strong overlap in molecular functionality between training and testing datasets, then it is reasonable to anticipate that aggregated estimates will be accurate.
Conversely, if training molecules do not accurately represent test molecules then we expect aggregate estimates to perform poorly.

This assumption however, is only partially correct.
Compound heterogeneity between training and testing datasets is actually a minor factor to consider when assessing estimate performance.
Additionally, this point of concern can be directly addressed through experimental procedures like the cross-validation tests performed in Section~\ref{Method:StatEnsemble:Design}.
These procedures assess the expected aggregate estimate's performance when estimating any compound in the dataset, \emph{regardless} of which molecules were used to train and which were used to test. 

When assessing an ensemble's performance, the more important attribute to characterize is the degree that ensemble constituents provide \emph{consistent} performance over varied chemical targets.
In general, it should not matter if there are large discrepancies between training and testing datasets, so long as the methods in the optimal ensemble are largely \emph{consistent} in their predictive performance: i.e., stronger performing methods in the training sets are also the stronger performing methods in the testing sets.
...
In returning to the performance seen in Figures~\ref{Analysis:Figure4:Compounds1} and~\ref{Analysis:Figure5:Compounds2}, the optimal ensemble outperforms one or both of its constituents in over 40 small molecule compounds.
The remaining molecules seem to challenge the ensemble's estimate: e.g., SAMPL4\_002, SAMPL4\_017, SAMPL4\_024, SAMPL4\_030, and SAMPL4\_047).
In these cases the optimal model is performing worse than both of its method constituents: alc-3 and imp-2.
In examining Figure~\ref{}



%For example, assume that both training and test sets consisted of small, aliphatic alkanes with minimal functional groups (esters, ethers, etc.).
%The uncertainties and variance in performance between methods on the training set should be close to those uncertainties found in the testing set.
%It is reasonable to assume that the learned aggregate model built on the training data should therefore provide excellent performance on the testing compounds. 
%
%Now assume that our testing set consists of larger, aromatic compounds such as imidazoles, thiazoles, and quinones that are quite different from the training data set. 
%We may initially assume that the aggregate estimate would perform poorly under these conditions: e.g., due to functional and molecular dissimilarities between testing and training molecules. 
%However, this may not necessarily be the case.  
%If the methods predicting hydration energies for aliphatic alkanes perform proportionally on the new aromatic testing compounds then the underlying aggregate model should perform very well. 
%However, if training behavior of the methods does not map well to the testing data, for example if the best performers in the training dataset are the worse performers in the validation set, then the aggregate's estimate can be expected to be perform poorly.
