...
\begin{abstract}
	...
	This work provides a new approach for accurate solvation free energy prediction and lays the foundation for future work on aggregate models that can balance computational cost with estimation accuracy.
\end{abstract}
...
Additionally, there are two types of ensembles that we consider in this work.
...
At a high level, the framework begins by first evaluating all methods collectively as an ensemble and uses the ensemble to make an aggregated estimate. 
...
While improved estimates are certainly an important objective for this framework, we are equally interested in providing a better interpretation of the statistical ``black box'' behind these aggregated estimates. 
...
Towards this objective, this framework provides quantitative information to characterize the optimal ensemble.  
For example, to help determine if the ensemble relied solely on implicit or explicit methods, or if it used variety of different methods.
To better understand which methods contributed the most useful information in the aggregate estimate.
Alternatively, if the ensemble is largely composed of one type of method, to better understand the subtle difference in the implementation of these methods (e.g., variations in small molecule force fields)? 
...
While some models provide accurate information that will boost the ensemble's predictive accuracy and robustness, many models will be misspecified (i.e, improperly constructed  or derived).
...
Bonferroni corrections help address the problems that occur due to multiple concurrent comparisons; failure to address these problems can result in rejecting $H_0$ when in fact the hypothesis should not be rejected.
...
\todo{LJG: Get some concrete examples from Nathan}
...
Rather, the model defines a \emph{linear relationship} between two very different approaches that can be used to test and refine these individual methods both independently and in-concert of each other.
...
Specifically, in drug discovery there tends to be a significant difference between randomly partitioning the data between training and validation sets (as was done in this work) and partitioning it in some other way: e.g., by molecule size, by discovery date, by polarity, etc. 
In future work we will investigate the application of this method to specific classes of small molecules to explore the limitations of aggregating methods selected as optimal on one class of compounds for a second, significantly different set of compounds.
Additionally, the BMA approach is purely statistical in nature, but we are working on modifying the aggregation process to leverage structural and environmental features (e.g., only look at ensembles of empirical methods for certain structural features and consider all methods for other structures).
Finally, we will are looking at penalizing computationally expensive methods that provide minimal accuracy benefits to optimize both the accuracy of the estimates and the efficiency of the calculation.
