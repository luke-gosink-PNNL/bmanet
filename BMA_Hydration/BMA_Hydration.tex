...
Madigan and Raftery~\cite{Madigan:1994} developed the Occam's Window ensemble-pruning approach that eliminates under-performing models based on \rev{the Bayesian Information Criteria (BIC) discussed above.}
...
\rev{The first stage, presented in Algorithm~\ref{Method:Alg2}, is the analysis stage and its function is to evaluate the statistical significance of each method in the current version of the ensemble via Equation~\ref{Method:E7}.
...
The algorithm will iterate over the following tasks while it still has more than two methods to evaluate (line 5 - line 19).}
...
An \rev{RMSE} was obtained for the 26 estimates made on validation data.
...
\rev{To control the family-wise error rate of our tests, we applied a Bonferroni correction to determine corrected $p$-values with a threshold of $\alpha = 0.05$.
...
\rev{In this work the overall performance of the aggregate estimate is overall better than its constituents (Table~\ref{Analysis:Figure1:Methods}, Figures~\ref{Analysis:Figure4:Compounds1} and~\ref{Analysis:Figure5:Compounds2},) and significantly better than other methods.
...
\rev{In this work, the overall performance of the aggregate estimate is very consistent across chemical compounds (Figures~\ref{Analysis:Figure4:Compounds1} and~\ref{Analysis:Figure5:Compounds2},) with the exception of SAMPL4\_001 and SAMPL4\_045.
...
\rev{The random forests approach is an ensemble learning method similar to BMA. It operates by constructing a multitude of decision trees at training time and outputs an estimate based on the mean prediction of the individual trees. 
...
\rev{The last two approaches, Lasso and forward selection, are both strategies for selecting methods to use in a linear regression model (Equation~\ref{Method:E1}). 
