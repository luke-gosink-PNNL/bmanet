...
\begin{abstract}
	...
	%DLM: ``predictive accuracy'' -- as per my e-mail comments, it's worth noting that you actually do no predictions here, and it's not clear to me how you would apply this approach predictively (i.e. what if SAMPL6 revisited hydration free energies and you wanted to apply this approach there?) so if you want to mention ``predictive'' you may need to discuss this in the paper.
\end{abstract}
...
Solvation free energy methods for atomically detailed molecular models have a long history of development \cite{Eisenberg:1986, Kang:1987:1, Kang:1987:2, Kang:1987:3,Kang:1987:4, Tan:2006, Gallicchio:2002,Baker:2013,Baker:2015} and have recently benefited from the curation of experimental small molecule solvation data for blind prediction challenges such as the Statistical Assessment of Modeling of Proteins and Ligands (SAMPL) challenge studies \cite{Nicholls:2008, Mobley:2009b, Klimovich:2010, Mobley:2009, Geballe:2012, Geballe:2010, Mobley:2014}.
...
%DLM: I think somewhere right around here you need to define how you are going to use the term `model' in this paper versus how you will use the term `method', as the Methods section and some of the material later hinges on the distinction between a `model' and a `method' and this is not made super obvious. You want to say exactly what they mean, i.e. ``Here, we use the term `method' to refer to a specific method (and associated set of parameters, settings, etc.) used to estimate solvation free energies, and the term `model' to refer to some approach for estimating solvation free energies that could be a weighted combination of estimates from one or more methods.'' You'll probably want to restate this in brief again later as well.
...
Specifically, if an aggregate estimate proves to be more accurate than individual methods, it is imperative to understand which methods are important to combine
%DLM: I don't totally understand this phrasing ``which methods are important to combine''; I think you're commenting on which methods carry information that is not captured by the other methods, or something along those lines? 
 and how they are combined so that domain knowledge may be obtained from the statistical aggregation process.
...
If the ensemble is largely of one type of method, what are the subtle difference in the implementation of these methods (e.g., variations in small molecule force fields)? 
...
%DLM: OK, so I see that you make this point about models vs methods here, though I'd note it's only a single sentence and is in the ``methods'' section which many readers consider optional. You probably have to make it more prominent since it's such a key point.
...
\end{equation}
The Bayesian Information Criteria (BIC) is a method for ranking models based on the model's goodness of fit, $R^2$.
The $R^2$ is a statistical measure of how close the data are to the fitted regression line.
It is basically the percentage of the response variable variation that is explained by a linear mode.
...
%DLM: How does this idea, penalizing models with more variables, come in to play here if at all? i.e., I don't even know the number of variables for most of the SAMPL methods... Is this paragraph just describing things which COULD be done? I think it needs to be made more clear.
%DLM: Later -- ah, OK, maybe this is a difference between a `method' and a `model' and you're here referring to MODELS with some number of parameters which average over methods; you are unconcerned with how many parameters the methods have. I'll suggest a terminology change earlier based on this. (I also found where you DO define this above, but it hadn't stuck with me when I read it. I'm leaving this here so you can get a sense of how people might get confused.)
...
%DLM: What does `misspecified' mean here?
...
%DLM: I don't understand the sentence just prior; is this giving a formal definition of ``utility''? And, are you saying the goal is that each model has nonzero coefficients for all methods? 
%Also, the ``can be used'' phrasing is not helpful as it doesn't actually say that this is what you did, it just says that one could do this. 
The combination of Equations~\ref{Method:E6} and ~\ref{Method:E7} therefore provide a statistical framework to support an iterative, statistical process for designing an ensemble that will provide the best combination of models and methods for estimating solvation free energy. %DLM: Why models AND methods? Aren't you just taking models and combining them into methods? This (and discussion later) makes it sound like you are making models which consist of other models and of methods.

%DLM: I don't totally understand this whole section just prior, at least not without digging in to the details and background. Superficially it sounds like you are saying that you want to not consider certain models (because doing so would degrade your accuracy) so you prioritize models which perform well. However, I don't understand how you can know which models will perform well until you know the answers. 

%DLM: Overall, I think the section reads as though it's written for people already very familiar with this area (which I'm not), and reminds me of the comment I made by e-mail that a major point of this work is to educate people on how to do this. This sentence doesn't really do any educating, or at least doesn't educate me. 
...
%DLM: Why is the first stage given in Algorithm 2? Seems odd.
%DLM: By e-mail, I had written this comment which applies to Algorithm 1 and Algorithm 2:
%It seems like a major point of this work is to educate people on how to do this type of thing/show that it can be done. If that's the case, I think more work needs to go into clarity of terminology/definitions/etc. Perhaps source code could also be provided. To be specific, I find the code in "Algorithm 1" and "Algorithm 2" very hard to read and there are many things there I don't understand. For example, what does the left arrow mean? What is "r" in algorithm 1? Why is the rmse set to infinity initially? I think it would be good to try and carefully edit the whole thing with an eye to how to explain to/show to a novice what this means and how to do it... (I could edit at that level if given enough time, but presumably there's not enough time for that right now.)
...
The second and third lines initialize the root mean squared error (RMSE) and the solution for the best ensemble of models and methods. %DLM: ``models and methods'' again.
...
\begin{table}[t] 
	\begin{minipage}[t]{0.45\linewidth}\centering 
		\begin{algorithm}[H] 
			...
			%DLM: This pseudocode needs help/clarification; see comments on Algorithm 2 below and apply here as well. Basically I just can't follow it yet.
			\begin{algorithmic}[1] 
				{\scriptsize 
	...
	\begin{minipage}[t]{0.45\linewidth}
		...
		\begin{algorithm}[H] 
			...
			\begin{algorithmic}[1] 
				{\scriptsize 
				%DLM: In this algorithm, you seem to have shifted what superscripts denote from denoting a particular model (in the Methods section) to denoting the dimensions of the matrix (??) here, if I read it right. You shouldn't do such a shift without stating it.
				\REQUIRE  \textbf{vector} $\textbf{y}^{n \times 1}$, \textbf{matrix} $\textbf{m}^{n \times j}: j \leq p$\; 
				%Could you say what matrix small m is, please? It doesn't seem to be stated in the text.
				\vspace{2.5mm} 
				\STATE $cv = 100$\; \COMMENT{100 cross-validation experiments}\; 
				\STATE $\beta_{BMA}^{1 \times j} = \textbf{0}^{1 \times j}$\; 
				%DLM: I have no idea what $\textbf{0}^{1 \times j}$ means. Is this some sort of array initialization?
				\STATE $error = 0$\; 
				\STATE $\textbf{v}^{1 \times j} = \textbf{0}^{1 \times j}$\; 
				%DLM: v is in boldface but beta, above, is not. Why? They're being set equal to the same thing. 
				\WHILE{$($$index$ $<$ $cv$$)$}\; 
					...
					%DLM: Why are t in train and v in validate in bold? 
					...
					%DLM: Why is this added to v? (And what is v?)
					\STATE $index = index + 1$ 
				\ENDWHILE\; 
...
%DLM: This is where my comment earlier (by e-mail) about how the data is partitioned applies -- normally in machine learning i.e. in drug discovery there is a huge difference between randomly partitioning the data between training and validation sets and partitioning it in some other way (by molecule size, by discovery date, by polarity, etc...) because it tends to be that in drug discovery, BY DESIGN, the later compounds are different than the former compounds, so you can't necessarily predict the later ones well from the former ones.  That is, the training and test sets are not drawn from the same distribution. In this study, you're deliberately making the training and test sets be drawn from the same distribution which will make your performance appear higher than it would in a predictive challenge (where your test set may be drawn from a different distribution). This will at least need to be discussed in the paper.
...
%DLM: So if you were applying this prospectively, how would you do it? Would you run a variety of methods and only include methods with low correlation between their results? What if correlation is because they are quite predictive of experiment rather than because they agree with one another?  Or would it require a human going through and including things which are ``different'' from one another regardless of whether the results agree or not?
...
%DLM: This paragraph confuses me, as does the mention of ``the validation set'' since it should be ``the validation setS'' if in fact a new validation set was selected each of the 100 trials. 
...
%DLM: Bonferroni -- again, if the purpose of this paper is to educate, you will probably want to say what this means.
...
Finally, a Shapiro-Wilk test~\cite{Shapiro:65} was used to test the hypothesis, $H_{0}$, that residuals in the SAMPL4 challenge are not significantly different from a normal population. %DLM: Say what Shapiro-Wilk is?
...
%DLM: "are ignored" -- i.e. assumptions of normality indicate that these are ignored? Or they indicate that we can ignore these, and so we do? 
...
%DLM: I don't understand the prior sentence. What's a predictive sensitivity? Are you saying that models do badly both because they can contain bad methods, and because there are uncertainties connected with how the models are built?
...
%DLM: I'm not sure what this means. Are you saying that if there were more big compounds included in new data, predictions would still be accurate because the model construction would pick methods which get these ``right'' and include them? What about if there are physical effects here that none of the models capture? (i.e. protonation state or tautomer changes in SAMPL5 for example) It seems to me that you can't get something for nothing, so this may be overoptimistic.
...
%DLM: WHat exactly do you mean by this last one? e.g. how would you use this idea? I'm not quite sure what you're getting at. You have a model that uses these two methods to estimate free energies, but then what? 
...
%DLM: I don't understand this last point either.
...
%DLM: This seems like where you would want to discuss how you would actually apply this in a new blind challenge -- i.e. imagine SAMPL6 was like SAMPL4 but on new compounds, what would YOU do with your approach to participate (rather than retrospectively analyze) in the challenge? 
